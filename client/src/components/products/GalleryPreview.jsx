...
const GalleryPreview = () => {
  const topSixImages = GalleryData ? GalleryData.slice(0, 6) : [];
  ...
  return (
    <section className="w-full py-16 lg:py-24 bg-white overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 flex flex-col items-center">
        ...
        <div className="flex flex-wrap justify-center w-full gap-y-6 md:gap-y-8 gap-x-4">
          {topSixImages.map((item, i) => (
            <div 
              key={`box-${i}`}
              className="w-full md:w-[48%] lg:w-[31%] h-64 md:h-72 lg:h-80 relative group rounded-2xl overflow-hidden shadow-lg cursor-pointer"
            >
        ...
        <div className="mt-12 lg:mt-16" data-aos="fade-up">
          <Link 
            to="/gallery"
            className="group flex items-center justify-center gap-3 bg-gradient-to-r from-red-600 to-orange-500 hover:from-red-700 hover:to-orange-600 text-white font-bold py-4 px-10 rounded-xl shadow-xl shadow-orange-500/20 transition-all duration-300"
          >
